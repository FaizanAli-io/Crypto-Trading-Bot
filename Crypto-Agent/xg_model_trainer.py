...
class FeatureModelTrainer:
    ...
    def __init__(self, model_root=None):
        ...
        # Use a dedicated output root to avoid conflicts with existing models
        resolved_root = Path(model_root) if model_root else Path("models_2025")
    ...
    def prepare_training_data(
        ...
        interval="1h",
        use_smc=False,  # Control SMC feature inclusion
        start_date=None,
    ):
        """
        ...
        df = self.data_collector.get_historical_data_by_date_range(
            symbol=symbol,
            start_date=start_date if start_date else "2025-08-01",
            days=days,
            interval=interval,
        )
        ...
        logger.info(
            f"  Up (1): {y_train.sum()} ({y_train.sum()/len(y_train)*100:.1f}%)"
        )
        logger.info(
            f"  Down (0): {len(y_train) - y_train.sum()} ({(1-y_train.sum()/len(y_train))*100:.1f}%)"
        )
    ...
    def train(self, X_train, y_train, X_test, y_test):
        ...
        logger.info(
            f"Class balance adjustment: scale_pos_weight={scale_pos_weight:.2f}"
        )
    ...
    def save_model(self, symbol="BTCUSDT"):
        ...
        model_path = (
            self.xg_models_dir
            / f"{smc_prefix}{symbol}_{self.interval}_{self.horizon_minutes}min_{timestamp}.pkl"
        )
        ...
        features_path = (
            self.features_dir
            / f"{smc_prefix}{symbol}_{self.interval}_{self.horizon_minutes}min_{timestamp}.txt"
        )
    ...
    def evaluate(self, X_train, y_train, X_test, y_test):
        ...
        logger.info(
            "\n"
            + classification_report(y_test, y_test_pred, target_names=["Down", "Up"])
        )
    ...
    def cross_validate(self, X, y, n_splits=5):
        ...
        logger.info(
            f"  Mean Accuracy: {np.mean(scores):.4f} (+/- {np.std(scores):.4f})"
        )
...
def train_currency(
    symbols, interval, horizon_minutes, use_smc=False, days=365, start_date=None
):
    """
    ...
    """
    effective_start = start_date if start_date else "2025-08-01"
    if isinstance(effective_start, str):
        ...
        raise ValueError("start_date must be string 'YYYY-MM-DD' or datetime")

    end_dt = start_dt + timedelta(days=days)
    end_date_str = end_dt.strftime("%Y-%m-%d")
    model_root = Path(f"models_{end_date_str}")

    trainer = FeatureModelTrainer(model_root=model_root)
    print(symbols)
    for symbol in symbols:
        ...
        logger.info(f"  SMC Features: {'ENABLED' if use_smc else 'DISABLED'}")
        logger.info(f"  Output folder: {model_root}")
...
def main():
    """Train five horizons for all configured symbols into models_2025"""

    logger.info("=" * 70)
    logger.info("TRAINING XGBOOST MODELS (2025 RUN)")
    logger.info("=" * 70)

    symbols = list(SUPPORTED_CRYPTOS.values())  # 7 symbols from config
    intervals = ["5m", "15m", "30m", "1h", "2h"]
    use_smc = False
    days = 365
    start_date = "2025-01-01"  # Train on full year 2025

    for interval in intervals:
        horizon_minutes = FeatureModelTrainer.interval_to_minutes(interval)
        logger.info(
            """
============================================
TRAINING BATCH
...
============================================
""".format(
                interval=interval, horizon_minutes=horizon_minutes, count=len(symbols)
            )
        )
