...
class DataCollector:
    ...
    def get_historical_data_by_date_range(
        ...
        """
        Get historical data between specific dates (starting at start_date and moving forward)

        Args:
            symbol (str): Trading pair (e.g., "ETHUSDT")
            start_date (str or datetime): Start date - format "YYYY-MM-DD" or datetime object
                                          Data collection begins at this date
            days (int): Number of days to collect starting from start_date
                    Example: start_date="2025-08-01", days=365
                    -> fetches from 2025-08-01 to 2026-08-01
        ...
        Example:
            # Get data from Aug 1, 2025 to Aug 1, 2026
        ...
        try:
            ...
            # Calculate end_date (going forward)
            end_date = begin_date + timedelta(days=days)

            logger.info(
                f"Fetching {symbol} data from {begin_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"
            )
            ...
            while current_start < end_timestamp:
                ...
                try:
                    ...
                    if not klines:
                        logger.warning(
                            f"No more data available after {datetime.fromtimestamp(current_start/1000)}"
                        )
                        break
