...
class IntegratedTradingSimulator:
    ...
    def simulate_from_backtest(
        ...
        timestamps = [
            datetime.fromisoformat(ts) for ts in backtest_results["timestamps"]
        ]
        ...
        if df_full is None:
            logger.warning(
                "Full dataframe not available, falling back to simple estimation"
            )
            df_full = None
        ...
        for i in range(len(predictions)):
            ...
            logger.info(
                f"[{i+1:2d}] {emoji} {timestamp.strftime('%Y-%m-%d %H:%M')} | "
        ...
        summary = {
            ...
            "avg_confidence": (
                np.mean([t["confidence"] for t in trades]) if trades else 0
            ),
        }
    ...
    def _estimate_target_price(self, df, current_price, direction, confidence, horizon):
        ...
        if len(df_recent) < horizon + 1:
            logger.warning(
                "Insufficient data for historical estimation, using simple method"
            )
            return self._estimate_target_price_simple(
                current_price, direction, confidence, horizon
            )
    ...
    def _estimate_target_price_simple(
        self, current_price, direction, confidence, horizon
    ):
        """
    ...
    def _print_summary(self, symbol, horizon, summary):
        ...
        logger.info(
            f"  {profit_emoji} {'PROFIT' if summary['total_return'] > 0 else 'LOSS'}"
        )

        logger.info(f"\nTRADING STATS:")
        logger.info(f"  Total Trades:         {summary['num_trades']}")
        logger.info(
            f"  Winning Trades:       {summary['winning_trades']} ({summary['win_rate']:.1f}%)"
        )
        ...
        if summary["best_trade"]:
            ...
            logger.info(f"\nBEST TRADE:")
            logger.info(
                f"  Trade #{best['trade_number']} | {best['type']} | "
        ...
        if summary["worst_trade"]:
            ...
            logger.info(f"\nWORST TRADE:")
            logger.info(
                f"  Trade #{worst['trade_number']} | {worst['type']} | "
