...
class SMCFeatureEngineer:
    ...
    def _add_order_blocks(self, df):
            ...
            if df["strong_up_move"].iloc[i]:
                ...
                for lookback in range(1, min(5, i)):
                    if df["close"].iloc[i - lookback] < df["open"].iloc[i - lookback]:
                        ...
                        df.loc[df.index[i], "ob_strength"] = (
                            abs(df["close"].iloc[i] - df["close"].iloc[i - lookback])
                            / df["close"].iloc[i - lookback]
                            * 100
                        )
                        break
        ...
        df["dist_to_bull_ob"] = (
            (df["close"] - df["bullish_ob_high"]) / df["close"]
        ) * 100
        df["dist_to_bear_ob"] = (
            (df["bearish_ob_low"] - df["close"]) / df["close"]
        ) * 100
    ...
    def _add_fair_value_gaps(self, df):
        ...
        df["price_in_fvg"] = (
            (df["close"] >= df["fvg_bottom"]) & (df["close"] <= df["fvg_top"])
        ).astype(int)
    ...
    def _add_liquidity_zones(self, df):
        ...
        df["dist_to_liquidity_high"] = (
            (df["liquidity_high"] - df["close"]) / df["close"]
        ) * 100
        df["dist_to_liquidity_low"] = (
            (df["close"] - df["liquidity_low"]) / df["close"]
        ) * 100
    ...
    def _add_market_structure(self, df):
        ...
        df["swing_high"] = (
            df["high"].rolling(window * 2 + 1, center=True).max() == df["high"]
        )
        df["swing_low"] = (
            df["low"].rolling(window * 2 + 1, center=True).min() == df["low"]
        )
    ...
    def _add_premium_discount_zones(self, df):
        ...
        df["at_equilibrium"] = (
            (df["range_position"] >= 0.45) & (df["range_position"] <= 0.55)
        ).astype(int)
    ...
    def _add_liquidity_sweeps(self, df):
        ...
        for i in range(window, len(df)):
            ...
            if df["high"].iloc[i] > max_recent and df["close"].iloc[i] < max_recent:
            ...
            if df["low"].iloc[i] < min_recent and df["close"].iloc[i] > min_recent:
    ...
    def _add_inducement_zones(self, df):
        ...
        for i in range(6, len(df)):
            ...
            if (
                df["close"].iloc[i - 3] > df["high"].iloc[i - 6 : i - 3].max()
                and df["close"].iloc[i] < df["close"].iloc[i - 3]
            ):
                df.loc[df.index[i], "inducement_long"] = 1

            # False bearish breakdown (inducement for shorts)
            if (
                df["close"].iloc[i - 3] < df["low"].iloc[i - 6 : i - 3].min()
                and df["close"].iloc[i] > df["close"].iloc[i - 3]
            ):
