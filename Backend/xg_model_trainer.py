...
class FeatureModelTrainer:
    ...
    def prepare_training_data(
        ...
        df = self.data_collector.get_historical_data_by_date_range(
            symbol=symbol, start_date="2025-08-01", days=days, interval=interval
        )
    ...
    def save_model(self, symbol="BTCUSDT"):
        ...
        model_path = (
            self.model_dir
            / f"xgb_model_{smc_prefix}{symbol}_{self.interval}_{self.horizon_minutes}min_{timestamp}.pkl"
        )
        ...
        features_path = (
            self.model_dir
            / f"features_{smc_prefix}{symbol}_{self.interval}_{self.horizon_minutes}min_{timestamp}.txt"
        )
        ...
        scaler_path = (
            self.model_dir
            / f"scaler_{smc_prefix}{symbol}_{self.interval}_{self.horizon_minutes}min_{timestamp}.pkl"
        )
